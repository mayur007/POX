...
#adding for circuit
  'OFPC_10G_WAN'      : 14680064,  
}

ofp_port_swtype_rev_map = {
  'OFPST_OTN'         : 1,
  'OFPST_WAVE'        : 8192,
  'OFPST_FIBRE'       : 16384,    
}

ofp_port_lam_bw_rev_map = {
  'OFPCBL_X'         : 1,
  'OFPCBL_100_50'    : 2,
  'OFPCBL_C_L'       : 4,    
}

ofp_connect_wildcards_rev_map = {
  'OFPCW_IN_PORT'    : 1,  
  'OFPCW_OUT_PORT'   : 2,  
  'OFPCW_IN_TPORT'   : 4, 
  'OFPCW_OUT_TPORT'  : 8,  
...
class ofp_header (ofp_base):
  ...
  def _unpack_header (self, raw, offset):
    global OF_VERSION_CKT
    offset,(self.version, self.header_type, length, self.xid) = \
        _unpack("!BBHL", raw, offset)
    OF_VERSION_CKT = self.version    
    #print ("packed header ver",self.version, "hdr type",self.header_type, \
	#	"verion aft", OF_VERSION_CKT)
    return offset,length
...
class ofp_phy_port (ofp_base):
  ...
  def unpack (self, raw, offset=0):
    _offset = offset
    #print ("PACKET: raw", len(raw), "offset", offset)
    offset,(self.port_no,) = _unpack("!H", raw, offset)
    #print ("PACKET:raw", len(raw), "offset", offset, "portno", self.port_no)
    offset,self.hw_addr = _readether(raw, offset)
    #print ("PACKET:raw", len(raw), "offset", offset, "hwaddr", self.hw_addr)
  ...
  def __repr__(self):
    return self.show()
#############################start of circuit port structs######################################

##2.1.1 circuit Port Structures
class ofp_phy_cport (ofp_base):
  def __init__ (self, **kw):
    self.port_no = 0
    self.hw_addr = EMPTY_ETH
    self.name = ""
    ...
    self.supported = 0
    self.peer = 0
    self.supp_sw_tdm_gran = 0
    self.supp_swtype = 0
    self.peer_port_no = 0
    self.peer_datapath_id = 0
    self.num_bandwidth = 0
    self.bandwidth  = 0
    initHelper(self, kw)
  ...
  def pack (self):
    ...
    packed += (self.hw_addr if isinstance(self.hw_addr, bytes) else
               self.hw_addr.toRaw())
    packed += self.name.ljust(OFP_MAX_PORT_NAME_LEN,'\0')
    packed += struct.pack("!LLLLLLL", self.config, self.state, self.curr,
                          self.advertised, self.supported, self.peer,self.supp_sw_tdm_gran)
    packed += struct.pack("!H", self.supp_swtype)
    packed += struct.pack("!H", self.peer_port_no)
    packed += struct.pack("!Q", self.peer_datapath_id)
    packed += struct.pack("!H", self.num_bandwidth)
    packed += _PAD6
    packed += struct.pack("!Q", self.bandwidth)
    return packed
  ...
  def unpack (self, raw, offset=0):
    ...
    offset,(self.port_no,) = _unpack("!H", raw, offset)
    offset,self.hw_addr = _readether(raw, offset)
    #print ("raw", len(raw), "offset", offset, "hwaddr", self.hw_addr,"portno.", self.port_no)
    offset,self.name = _readzs(raw, offset, OFP_MAX_PORT_NAME_LEN)
    offset,(self.config, self.state, self.curr, self.advertised,
            self.supported, self.peer, self.supp_sw_tdm_gran) = _unpack("!LLLLLLL", raw, offset)
    offset,(self.supp_swtype, self.peer_port_no, self.peer_datapath_id, self.num_bandwidth) = _unpack("!HHQH", raw, offset)
   # print ("portname", self.name, "swtype", self.supp_swtype,"numbw",self.num_bandwidth)
    offset = _skip(raw, offset, 6)
    offset,(self.bandwidth,) = _unpack("!Q", raw, offset)
    assert offset - _offset == len(self)
    return offset
  ...
  def __eq__ (self, other):
    ...
    if self.supported != other.supported: return False
    if self.peer != other.peer: return False
    if self.supp_sw_tdm_gran != other.supp_sw_tdm_gran: return False
    if self.supp_swtype != other.supp_swtype: return False
    if self.peer_port_no != other.peer_port_no: return False
    if self.peer_datapath_id != other.peer_datapath_id: return False
    if self.num_bandwidth != other.num_bandwidth: return False
  ...
  def __hash__(self, *args, **kwargs):
    return hash(self.port_no) ^ hash(self.hw_addr) ^ \
           ...
           hash(self.state) ^ hash(self.curr) ^ \
           hash(self.advertised) ^ hash(self.supported) ^ \
           hash(self.peer) ^ hash(self.supp_sw_tdm_gran) ^ hash(self.supp_swtype) ^ \
	   hash(self.peer_port_no) ^ hash(self.peer_datapath_id) ^ \
	   hash(self.num_bandwidth) + hash(self.bandwidth)
  ...
  def show (self, prefix=''):
    ...
    outstr += prefix + 'supported: ' + str(self.supported) + '\n'
    outstr += prefix + 'peer: ' + str(self.peer) + '\n'
    outstr += prefix + 'supp_sw_tdm_gran: ' + str(self.supp_sw_tdm_gran) + '\n'
    outstr += prefix + 'supp_swtype: ' + str(self.supp_swtype) + '\n'    
    outstr += prefix + 'peer_port_no: ' + str(self.peer_port_no) + '\n'
    outstr += prefix + 'peer_datapath_id: ' + str(self.peer_datapath_id) + '\n'
    outstr += prefix + 'num_bandwidth: ' + str(self.num_bandwidth) + '\n'
    outstr += prefix + 'bandwidth: ' + str(self.bandwidth) + '\n'
    return outstr
...
##2.1.2 wave Port Structures
class ofp_wave_port (ofp_base):
  def __init__ (self, **kw):
    self.wport = 0
    self.wavelength = 0
    initHelper(self, kw)
  ...
  def pack (self):
    ...
    packed = b""
    packed += struct.pack("!H", self.wport)
    packed += _PAD6
    packed += struct.pack("!Q", self.wavelength)
    return packed

  def unpack (self, raw, offset=0):
    _offset = offset
    #print ("PACKET: raw", len(raw), "offset", offset)
    offset,(self.wport,) = _unpack("!H", raw, offset)
    offset = _skip(raw, offset, 6)
    #print ("PACKET:raw", len(raw), "offset", offset, "wport", self.wport)
    offset,(self.wavelength) = _unpack("!Q", raw, offset)
    assert offset - _offset == len(self)
    return offset
  ...
  def __eq__ (self, other):
    if type(self) != type(other): return False
    if self.wport != other.wport: return False
    if self.wavelength != other.wavelength: return False
    return True

  def __cmp__ (self, other):
    if type(other) != type(self): return id(self)-id(other)
    if self.wport < other.wport: return -1
    if self.wport > other.wport: return 1
    if self == other: return 0
    return id(self)-id(other)
  
  def __hash__(self, *args, **kwargs):
    return hash(self.wport) + hash(self.wavelength) 

  def show (self, prefix=''):
    outstr = ''
    outstr += prefix + 'wport: ' + str(self.wport) + '\n'
    outstr += prefix + 'wavelength: ' + str(self.wavelength) + '\n'
    return outstr

  def __repr__(self):
    return self.show()

#############################end of circuit port structs######################################
...
#####################################circuit addon connect#############################
##2.3 circuit connect (Flow Match) Structures
class ofp_connect (ofp_base):
  _MIN_LENGTH = 8
  def __init__ (self, **kw):
    self.wildcards = 0
    self.num_components = 0
    #remember the following has to be structs \
	#since we dont use them taking face values e.g. ofp_tdm_prt is 64bit/8bytes
    self.in_port = 0
    self.out_port = 0
    self.in_tport = 0
    self.out_tport = 0
    ...
    if not hasattr(self.in_wport, '__getitem__'):
      self.in_wport = [self.in_wport]

    #self.in_wport = ofp_wave_port()
    #self.out_wport = ofp_wave_port()
    '''
    if 'in_wport' in kw:
      self.in_wport = None
      self.out_wport = None
  ...
  def pack (self, cflow_mod=False):
    assert self._assert()

    packed = b""
    
    packed += struct.pack("!HH", self.wildcards, self.num_components)				
    packed += _PAD4
    if self.wildcards != 3840:
      pass
    #packed += struct.pack("!HH",self.in_port or 0, self.out_port or 0)
    #packed += struct.pack("!QQ",self.in_tport or 0, self.out_tport or 0)
    #packed += self.in_wport.pack()
    #packed += self.out_wport.pack()    

    for i in self.in_wport:
      print("no of in_wports:", i)
      packed += i.pack()    

    for i in self.out_wport:
      print("no of out_wports:", i)
      packed += i.pack()

    return packed

  def unpack (self, raw, offset=0, cflow_mod=False):
    _offset = offset
    offset,(wildcards, self.num_components) = _unpack("!HH",raw, offset)
    offset = _skip(raw, offset, 4)
    #print ("PACKET: raw", len(raw), "offset", offset)
    offset,(self.in_port, self.out_port) = _unpack("!HH",raw, offset)
    offset,(self.in_tport, self.out_tport) = _unpack("!QQ",raw, offset)

    print ("PACKET:raw", len(raw), "offset", offset)
    #get remaining raw buf, divide by length of wave port. 
    #get first set of ports in ports in portcount
    portCount = (len(raw) / len(ofp_wave_port))/2

    self.in_wport = []
    for i in xrange(0, portCount):
      p = ofp_wave_port()
      offset = p.unpack(raw, offset)
      self.in_wport.append(p)

    self.out_wport = []
    for i in xrange(0, portCount):
      p = ofp_wave_port()
      offset = p.unpack(raw, offset)
      self.out_wport.append(p)
  ...
  def __eq__ (self, other):
    if type(self) != type(other): return False
    if self.wildcards != other.wildcards: return False
    if self.num_components != other.num_components: return False
    if self.in_port != other.in_port: return False
    if self.out_port != other.out_port: return False
    if self.in_tport != other.in_tport: return False
    if self.out_tport != other.out_tport: return False
    if self.in_wport != other.in_wport: return False
    if self.out_wport != other.out_wport: return False
    return True
  ...
  def show (self, prefix=''):
    outstr = ''
    outstr += prefix + 'wildcards: \n'
    outstr += prefix + 'num_components: ' + str(self.num_components) + '\n'
    outstr += prefix + 'in_port: ' + str(self.in_port) + '\n'
    outstr += prefix + 'out_port: ' + str(self.out_port) + '\n'
    outstr += prefix + 'in_tport: ' + str(self.in_tport) + '\n'
    outstr += prefix + 'out_tport: ' + str(self.out_tport) + '\n'
    
    outstr += prefix + 'in_wport: \n'  
    for obj in self.in_wport:
      outstr += obj.show(prefix + '  ')
    outstr += prefix + 'in_wport: \n'  
    for obj in self.out_wport:
      outstr += obj.show(prefix + '  ')
    return outstr
...
class ofp_features_reply (ofp_header):
  ...
  def pack (self):
    ...
    packed += struct.pack("!QLBB", self.datapath_id, self.n_buffers,
                          self.n_tables, self.n_cports)
    packed += _PAD2
    packed += struct.pack("!LL", self.capabilities, self.actions)
    for i in self.ports:
      packed += i.pack()
    
    for i in self.cports:
      packed += i.pack()
    return packed
  ...
  def unpack (self, raw, offset=0):    
    ...
    offset,length = self._unpack_header(raw, offset)
    #print("unpacking OF version:",(OF_VERSION_CKT) )
    offset,(self.datapath_id, self.n_buffers, self.n_tables, self.n_cports) = \
        _unpack("!QLBB", raw, offset)
    offset = _skip(raw, offset, 2)
    offset,(self.capabilities, self.actions) = _unpack("!LL", raw, offset)
    
    
    if (self.n_cports < 0 or (OF_VERSION_CKT != 3)):
        portCount = (length - 32) / len(ofp_phy_port)
        self.ports = []
        for i in xrange(0, portCount):
            p = ofp_phy_port()
            offset = p.unpack(raw, offset)
            self.cports.append(p)
    else:
    	portCount = (length - 32) / len(ofp_phy_cport)
	#print("portcount", portCount, "len(raw)", (len(raw) - 32))
    	self.cports = []
    	for i in xrange(0, portCount):
      	    p = ofp_phy_cport()
      	    offset = p.unpack(raw, offset)
  ...
  def __len__ (self):
    return 32 + len(self.cports) * len(ofp_phy_cport) + len(self.ports) * len(ofp_phy_port)
  ...
  def __eq__ (self, other):
    ...
    if self.n_cports != other.n_cports: return False
    if self.capabilities != other.capabilities: return False
    if self.actions != other.actions: return False  
    if self.ports != other.ports: return False  
    if self.cports != other.cports: return False
    return True
  ...
  def show (self, prefix=''):
    ...
    outstr += prefix + 'n_cports: ' + str(self.n_cports) + '\n'
    ...
    outstr += prefix + 'cports: \n'
    for obj in self.cports:
      outstr += obj.show(prefix + '  ')
    return outstr
...
###########################circuit addons cflow######################################
##3.3.1 Modify State Messages

@openflow_c_message("OFPT_CFLOW_MOD", 23)
class ofp_cflow_mod (ofp_header):
  _MIN_LENGTH = 24
  def __init__ (self, **kw):
    ofp_header.__init__(self)
    '''
  ...
  def _validate (self):
    if not isinstance(self.connect, ofp_connect):
      return "match/connect is not class ofp_connect"
    return None
  ...
  def pack (self):
    ...
    May normalize fields.
    NOTE: If "data" has been specified, this method may actually return
          *more than just a single ofp_cflow_mod* in packed form.
          Specifically, it may also have a barrier and an ofp_packet_out.
    """
    po = None
    ...
    packed = b""
    packed += ofp_header.pack(self)
    packed += struct.pack("!HH", self.command, self.hard_timeout)
    packed += _PAD4
    packed += self.connect.pack(cflow_mod=True)    
  ...
  def unpack (self, raw, offset=0):
    offset,length = self._unpack_header(raw, offset)
    offset,(self.command, self.hard_timeout) = \
            _unpack("!HH", raw, offset)
    offset = _skip(raw, offset, 4)
    offset = self.connect.unpack(raw, offset, cflow_mod=True)    
    offset,self.actions = _unpack_actions(raw,
        length-(16 + len(self.connect)), offset)
    assert length == len(self)
    return offset,length

  def __len__ (self):
    #l = 16 + len(ofp_connect)
    l = 16 + 8
    for i in self.actions:
      l += len(i)
    return l
  ...
  def __eq__ (self, other):
    if type(self) != type(other): return False
    if not ofp_header.__eq__(self, other): return False
    if self.connect != other.connect: return False
    if self.command != other.command: return False
    if self.hard_timeout != other.hard_timeout: return False    
    if self.actions != other.actions: return False
    return True
  ...
  def show (self, prefix=''):
    ...
    outstr += prefix + 'header: \n'
    outstr += ofp_header.show(self, prefix + '  ')
    outstr += prefix + 'connect: \n'
    outstr += self.connect.show(prefix + '  ')
    outstr += prefix + 'command: ' + str(self.command) + '\n'
    outstr += prefix + 'hard_timeout: ' + str(self.hard_timeout) + '\n'
    outstr += prefix + 'actions: \n'
    for obj in self.actions:
...
####################circuit addons################################
@openflow_s_message("OFPT_CPORT_STATUS", 22)
class ofp_port_status (ofp_header):
  def __init__ (self, **kw):
    ofp_header.__init__(self)
    self.reason = 0
    self.desc = ofp_phy_cport()

    initHelper(self, kw)

  def _validate (self):
    if not isinstance(self.desc, ofp_phy_cport):
      return "desc is not class ofp_phy_cport"
    return None
