...
def handle_FEATURES_REPLY (con, msg):
  ...
  def finish_connecting (event):    
    if event.xid != barrier.xid and event.xid != 10001:      
      con.dpid = None
...
#Added for circuit
def handle_CPORT_STATUS (con, msg): #A
  if msg.reason == of.OFPPR_DELETE:
    con.ports._forget(msg.desc)
  else:
    con.ports._update(msg.desc)
...
handlerMap = {
  ...
  of.OFPT_CPORT_STATUS : handle_CPORT_STATUS,
...
class Connection (EventMixin):
  ...
  def read (self):
    ...
    while buf_len - offset >= 8: # 8 bytes is minimum OF message size
      ...
      check_version = ord(self.buf[offset])	
	
      if check_version == 0 and ofp_type == 0:
	return True

      #print("reading socket ofptype", ofp_type, "ofpversion", check_version)	

      #if ord(self.buf[offset]) != of.OFP_VERSION:
      if check_version != of.OFP_VERSION:
	if ofp_type == 0:
	  pass
        elif ofp_type == of.OFPT_HELLO:
          # We let this through and hope the other side switches down.
          pass
	elif ord(self.buf[offset]) == of.OFPT_CKT:
	  # let ckt features identification through
	  pass	
        else:
          log.warning("Bad OpenFlow version (0x%02x) on connection %s"
                      % (ord(self.buf[offset]), self))
	  print("actual version", ord(self.buf[offset]))
          ...
          for con in rlist:
            if con is listener:
              ...
              if con.read() is False:
                con.close()
		log.error("close connection@Of_01_Task.")
                sockets.remove(con)
